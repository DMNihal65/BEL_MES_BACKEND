...
from app.models import ShiftHoursConfiguration
...
def is_working_day(dt: datetime) -> bool:
    """
    Check if the given datetime falls on a working day using ShiftHoursConfiguration table.
    Returns True if the date is marked as working_day in the configuration.
    Falls back to checking Monday-Saturday if no configuration exists for that date.
    """
    date_only = dt.date()

    try:
        shift_config = ShiftHoursConfiguration.select(lambda sc: sc.date.date() == date_only).first()
        if shift_config:
            return shift_config.working_day
    except Exception as e:
        print(f"Error checking ShiftHoursConfiguration for {date_only}: {str(e)}")

    # Fallback: Check if Monday-Saturday (6 is Sunday)
    return dt.weekday() != 6
...
def schedule_operations(df: pd.DataFrame, component_quantities: Dict[Tuple[str, str], int],
    ...
    with db_session:
        ...
        for order in Order.select():
            ...
            production_orders[order.part_number].append({
                ...
                'order_id': order.id,
                'priority': order.project.priority if (order.project and order.project.priority is not None) else float('inf'),
    ...
    for (partno, production_order) in active_parts.keys():
        ...
        if orders:
            ...
            # Handle NULL priorities by treating them as float('inf') (lowest priority)
            highest_priority = min([
                order.project.priority if (order.project and order.project.priority is not None) else float('inf')
                for order in orders
            ], default=float('inf'))
