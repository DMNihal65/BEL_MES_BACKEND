...
@router.post("/mpp/upload/", response_model=DocumentResponse)
async def upload_mpp_document(
        file: UploadFile = File(...),
        production_order: str = Form(...),
        document_name: str = Form(...),
        description: Optional[str] = Form(None),
    ...
    """Upload an MPP document for a specific production order"""
    try:
        # Process data outside db session
        metadata_dict = json.loads(metadata) if metadata else {}
        file_contents = await file.read()
        ...
        with db_session:
            ...
            # Get or create MPP document type
            doc_type = DocType.get(type_name="MPP")
            if not doc_type:
                doc_type = DocType(
                    type_name="MPP",
                    description="Manufacturing Process Plan",
                    file_extensions=[".pdf", ".doc", ".docx"],
                    is_active=True
                )
                flush()
            ...
            if file_ext not in [ext.lower().strip('.') for ext in doc_type.file_extensions]:
                raise HTTPException(
                    status_code=400,
                    detail=f"File type .{file_ext} not allowed for MPP documents"
                )

            # Get or create MPP folder
            mpp_folder = DocFolder.get(folder_name="MPP")
            if not mpp_folder:
                raise HTTPException(status_code=404, detail="MPP folder not found")

            # Create document with initial MinIO path
            temp_object_name = f"{production_order}/MPP/temp"
...
@router.get("/mpp/{production_order}", response_model=List[DocumentResponse])
async def get_mpp_documents(
        production_order: str,
        current_user: User = Depends(get_current_user)
):
    """Get all MPP documents for a production order"""
    try:
        with db_session:
            user = User[current_user.id]
            ...
            # Get MPP document type
            doc_type = DocType.get(type_name="MPP")
            if not doc_type:
                return []

            # Get all MPP documents for this order
            documents = select(d for d in Document
                               if d.is_active and
                               d.part_number_id == order and
                               d.doc_type == doc_type
...
@router.post("/engineering-drawing/upload/", response_model=DocumentResponse)
async def upload_engineering_drawing(
        file: UploadFile = File(...),
        production_order: str = Form(...),
        document_name: str = Form(...),
        description: Optional[str] = Form(None),
    ...
    """Upload an engineering drawing for a specific production order"""
    try:
        # Process data outside db session
        metadata_dict = json.loads(metadata) if metadata else {}
        file_contents = await file.read()
        ...
        with db_session:
            try:
                ...
                # Get or create Engineering Drawing document type
                doc_type = DocType.get(type_name="Engineering Drawing")
                if not doc_type:
                    doc_type = DocType(
                        type_name="Engineering Drawing",
                        description="Engineering Drawings and Specifications",
                        file_extensions=[".pdf", ".dwg", ".dxf"],
                        is_active=True
                    )
                    flush()
                ...
                if file_ext not in [ext.lower().strip('.') for ext in doc_type.file_extensions]:
                    raise HTTPException(
                        status_code=400,
                        detail=f"File type .{file_ext} not allowed for Engineering Drawings"
                    )

                # Get Engineering Drawings folder
                drawings_folder = DocFolder.get(folder_name="Engineering Drawings")
                if not drawings_folder:
                    raise HTTPException(status_code=404, detail="Engineering Drawings folder not found")

                # Create document with initial MinIO path
                temp_object_name = f"{production_order}/Engineering_Drawing/temp"
...
@router.get("/engineering-drawing/{production_order}", response_model=List[DocumentResponse])
async def get_engineering_drawings(
        production_order: str,
        current_user: User = Depends(get_current_user)
):
    """Get all engineering drawings for a production order"""
    try:
        with db_session:
            user = User[current_user.id]
...
@router.post("/{document_id}/versions/new", response_model=DocumentVersionResponse)
...
@router.get("/download/{doc_type}/{production_order}")
async def download_latest_document_by_type(
        doc_type: str,
        production_order: str,
        current_user: User = Depends(get_current_user)
):
    """Download the latest version of a specific document type for a production order"""
    try:
        with db_session:
            # Get necessary data within the session
            user = User[current_user.id]
            if not user:
                raise HTTPException(status_code=404, detail="User not found")
            ...
            doc_type_obj = DocType.get(type_name=doc_type)
            if not doc_type_obj:
                raise HTTPException(status_code=404, detail=f"Document type '{doc_type}' not found")

            # Get the most recent active document and its data
            ...
            if not document:
                raise HTTPException(
                    status_code=404,
                    detail=f"No {doc_type} document found for production order {production_order}"
                )
        ...
        # Get file from MinIO outside the db session
        try:
            file_stream = minio_service.get_file(minio_object_id)
