...
async def update_order_priority(order_id: int, priority_data: ProjectPriorityUpdateRequest):
    ...
    try:
        with db_session:
            ...
            # Get all projects ordered by priority (NULLs will be last)
            all_projects = select(p for p in Project).order_by(Project.priority)[:]

            # Handle case where old_priority is None (treat as lowest priority)
            if old_priority is None:
                # Moving from NULL to a specific priority
                # Shift down all projects with priority <= new_priority
                for project in all_projects:
                    if project.id != current_project.id and project.priority is not None:
...
def delete_order(order_id: int):
    ...
    if project_ref:
        ...
        if not project_ref.orders:
            ...
            project_ref.delete()
            # Rearrange priorities for remaining projects (only if deleted project had a priority)
            if deleted_priority is not None:
                for proj in Project.select(lambda p: p.priority is not None and p.priority > deleted_priority):
                    proj.priority -= 1
...
from app.models.master_order import ShiftHoursConfiguration
...
@router.get("/shift-hours", response_model=List[ShiftHoursConfigResponse])
@db_session
def get_all_shift_configs():
    configs = ShiftHoursConfiguration.select()[:]  # fetch all
    return list(configs)


@router.get("/shift-hours/{config_id}", response_model=ShiftHoursConfigResponse)
@db_session
def get_shift_config(config_id: int):
    config = ShiftHoursConfiguration.get(id=config_id)
    if not config:
        raise HTTPException(status_code=404, detail="Shift configuration not found")
    return config


@router.put("/shift-hours/{config_id}", response_model=ShiftHoursConfigResponse)
@db_session
def update_shift_config(config_id: int, update_data: ShiftHoursConfigUpdate):
    config = ShiftHoursConfiguration.get(id=config_id)
    if not config:
        raise HTTPException(status_code=404, detail="Shift configuration not found")

    if update_data.working_day is not None:
        config.working_day = update_data.working_day
    if update_data.no_of_shifts is not None:
        config.no_of_shifts = update_data.no_of_shifts

    commit()
    return config


@router.delete("/shift-hours/{config_id}", status_code=status.HTTP_204_NO_CONTENT)
@db_session
def delete_shift_config(config_id: int):
    config = ShiftHoursConfiguration.get(id=config_id)
    if not config:
        raise HTTPException(status_code=404, detail="Shift configuration not found")
    config.delete()
