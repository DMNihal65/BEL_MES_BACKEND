...
async def schedule():
    ...
    try:
        ...
        with db_session:
            # OPTIMIZATION 1: Single query to fetch all work centers with their machines
            work_centers = WorkCenter.select().prefetch(WorkCenter.machines)[:]
            schedulable_work_center_ids = set()
            ...
            for work_center in work_centers:
                ...
                if work_center.is_schedulable:
                    schedulable_work_center_ids.add(work_center.id)

            print(f"Schedulable work center IDs: {schedulable_work_center_ids}")

            # OPTIMIZATION 2: Single query for active production orders
            active_part_statuses = select(p for p in PartScheduleStatus if p.status == 'active')[:]

            if not active_part_statuses:
                print("No active production orders found")
                return ScheduleResponse(
                    scheduled_operations=[],
                    overall_end_time=datetime.utcnow(),
            ...
            # OPTIMIZATION 3: Process all mappings in single loop
            active_production_orders_set = set()
            po_to_part_mapping = {}
            po_to_part_description_mapping = {}
            part_po_to_quantity = {}

            # Get all part numbers for batch query
            part_numbers = {part_status.part_number for part_status in active_part_statuses}

            # OPTIMIZATION 4: Batch query for all orders
            orders_dict = {}
            orders = select(o for o in Order if o.part_number in part_numbers)[:]
            for order in orders:
            ...
            for part_status in active_part_statuses:
                ...
                active_production_orders_set.add(po)
                po_to_part_mapping[po] = part_number

                # Get order data from pre-fetched dictionary
                order = orders_dict.get((part_number, po))
                quantity = order.launched_quantity if order and order.launched_quantity > 0 else 10

                # Store the part description
                po_to_part_description_mapping[po] = order.part_description if order and order.part_description else ""
            ...
            # OPTIMIZATION 5: Single optimized query for operations
            # Get all operations for active parts and schedulable work centers in one query
            operations = select(
                o for o in Operation
                if o.order.part_number in part_numbers and
                o.order.production_order in active_production_orders_set and
                o.work_center.id in schedulable_work_center_ids and
                o.work_center.is_schedulable == True
            ).prefetch(Operation.order, Operation.machine, Operation.work_center)[:]

            if not operations:
                print("No operations found for active production orders in schedulable work centers")
                return ScheduleResponse(
                    scheduled_operations=[],
                    overall_end_time=datetime.utcnow(),
                    overall_time="0",
                    ...
                    component_status={},
                    partially_completed=[
                        "No operations found for active production orders in schedulable work centers"],
                    work_centers=work_centers_data
                )

            # OPTIMIZATION 6: Build dataframe directly from operations without multiple filters
            expanded_rows = []
            for op in operations:
                part_number = op.order.part_number
                po = op.order.production_order

                # Double check this combination exists in our quantity mapping
                if (part_number, po) in part_po_to_quantity:
                    expanded_rows.append({
                        'partno': part_number,
                        'operation': op.operation_description,
                        'machine_id': op.machine.id,
            ...
            if not expanded_rows:
                print("No valid operations after processing")
                return ScheduleResponse(
                    scheduled_operations=[],
                    overall_end_time=datetime.utcnow(),
                    overall_time="0",
            ...
            # Create dataframe from processed operations
            df = pd.DataFrame(expanded_rows)

        # Rest of the processing remains the same but now uses pre-filtered data
        print(f"Operations dataframe shape after optimization: {df.shape}")

        # Get the active part numbers based on the dataframe
        active_part_numbers_in_df = df['partno'].unique().tolist()
        print(f"Active part numbers in dataframe: {active_part_numbers_in_df}")

        # Create component_quantities dictionary - now using pre-computed quantities
        component_quantities = {}
        for _, row in df.iterrows():
            part_number = row['partno']
            production_order = row['production_order']
        ...
        # Get lead times and filter for active parts only
        lead_times = fetch_lead_times()
        lead_times = {k: v for k, v in lead_times.items() if k in active_part_numbers_in_df}
        ...
        # Verification and filtering (optimized)
        if not schedule_df.empty:
            print(f"Final schedule has {len(schedule_df)} operations")
            scheduled_pos = set(schedule_df['production_order'].unique())
            print(f"Production orders in final schedule: {list(scheduled_pos)}")

            # Filter out any operations with inactive production orders (safety check)
            schedule_df = schedule_df[schedule_df['production_order'].isin(active_production_orders_set)]

        # Filter component_status efficiently
        filtered_component_status = {}
        for key, status in component_status.items():
            production_order = None
        ...
        # Store schedule if not empty
        stored_schedule = None
        if not schedule_df.empty:
            with db_session:
                stored_schedule = store_schedule(schedule_df, component_status)

        # OPTIMIZATION 7: Pre-fetch machine details once
        scheduled_operations = []
        if not schedule_df.empty:
            with db_session:
                # Single query for all machine details
                machines = Machine.select().prefetch(Machine.work_center)[:]
                machine_details = {}
            ...
            for _, row in schedule_df.iterrows():
                ...
                # Skip if production order is not active (safety check)
                if production_order not in active_production_orders_set:
                    continue
