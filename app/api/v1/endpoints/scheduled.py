...
from app.models.master_order import Project
...
from app.models.master_order import OrderCompleted, ShiftHoursConfiguration
...
async def schedule():
    ...
    try:
        ...
        # Assign priorities to active orders before scheduling
        # First, set inactive orders to NULL, then assign priorities to active orders, then reorder sequentially
        with db_session:
            # Get all orders to update their project priorities
            all_orders = select(o for o in Order)[:]
            
            # Step 1: Set inactive orders to NULL
            for order in all_orders:
                if order.project and order.production_order not in active_production_orders_set:
                    order.project.priority = None
            
            # Step 2: Get max priority from remaining active projects (after setting inactive to NULL)
            max_priority = select(max(p.priority) for p in Project if p.priority is not None).first() or 0
            
            # Step 3: Assign priorities to active orders that don't have one
            for order in all_orders:
                if order.project and order.production_order in active_production_orders_set:
                    if order.project.priority is None:
                        max_priority += 1
                        order.project.priority = max_priority
            
            # Step 4: Reorder all active orders to have sequential priorities (1, 2, 3, ...)
            active_orders_with_projects = [
                order for order in all_orders 
                if order.project and order.production_order in active_production_orders_set
            ]
            
            # Sort by current priority (NULLs go last), then by project ID for consistency
            active_orders_with_projects.sort(
                key=lambda o: (
                    o.project.priority if o.project.priority is not None else float('inf'),
                    o.project.id
                )
            )
            
            # Assign sequential priorities starting from 1
            for index, order in enumerate(active_orders_with_projects, start=1):
                order.project.priority = index
...
def get_machine_utilization_by_range(
    ...
    Calculates:
    - Available hours: From ShiftHoursConfiguration (no_of_shifts * 8) * 0.85 efficiency for working days
    - Utilized hours: Sum of scheduled time from planned schedule items for active production orders
    - Remaining hours: Available - Utilized
    """
    if start_date > end_date:
        raise HTTPException(status_code=400, detail="End date must be after start date")

    # Fetch shift configuration for the date range
    shift_configs = select(sc for sc in ShiftHoursConfiguration
                           if sc.date >= start_date.replace(hour=0, minute=0, second=0, microsecond=0)
                           and sc.date < end_date.replace(hour=0, minute=0, second=0, microsecond=0) + timedelta(
        days=1))[:]

    # Create a map of date -> shift configuration for quick lookup
    shift_config_map = {}
    total_shift_hours = 0
    working_days_count = 0

    for config in shift_configs:
        config_date = config.date.replace(hour=0, minute=0, second=0, microsecond=0)
        shift_config_map[config_date] = config

        if config.working_day:
            shift_hours = config.no_of_shifts * 8  # Each shift is 8 hours
            total_shift_hours += shift_hours
            working_days_count += 1

    # Calculate base available hours with efficiency factor
    efficiency_factor = 0.85
    base_available_hours = total_shift_hours * efficiency_factor
    ...
    # Pre-fetch machine status records for the date range
    range_end = end_date.replace(hour=0, minute=0, second=0, microsecond=0) + timedelta(days=1)
    machine_statuses = select(ms for ms in MachineStatus
                              if ms.available_from is not None
                              and (ms.available_from < range_end and
                                   (ms.available_to is None or ms.available_to > start_date)))[:]

    # Group machine statuses by machine ID
    machine_status_map = {}
    for ms in machine_statuses:
        if ms.machine.id not in machine_status_map:
            machine_status_map[ms.machine.id] = []
    ...
    # Sort statuses by available_from
    for mid in machine_status_map:
        machine_status_map[mid].sort(key=lambda x: x.available_from)

    # Pre-fetch schedule items for the date range
    all_schedule_items = select(p for p in PlannedScheduleItem
                                if p.order.production_order in active_production_orders
                                and ((p.initial_start_time >= start_date and p.initial_start_time < range_end) or
                                     (p.initial_end_time > start_date and p.initial_end_time <= range_end) or
    ...
    # Group schedule items by machine ID
    schedule_items_map = {}
    for item in all_schedule_items:
        if item.machine.id not in schedule_items_map:
            schedule_items_map[item.machine.id] = []
    ...
    for machine in machines:
        ...
        # Get machine status records for this machine
        machine_statuses_list = machine_status_map.get(machine.id, [])

        # Process downtime periods
        status_index = 0
        while current_time < range_end:
            if status_index >= len(machine_statuses_list):
                break

            # Find applicable status
            while status_index < len(machine_statuses_list):
                status = machine_statuses_list[status_index]
                if status.available_from <= current_time:
                    if status.available_to is None or status.available_to > current_time:
            ...
            if status_index < len(machine_statuses_list) and machine_statuses_list[
                status_index].status.id == 2:  # Machine OFF
                current_status = machine_statuses_list[status_index]
        ...
        # Calculate utilized hours from schedule items
        schedule_items = schedule_items_map.get(machine.id, [])
        utilized_hours = 0
        daily_hours = {}
        ...
        for item in schedule_items:
            ...
            while current_day < end_day:
                ...
                # Get shift configuration for this day to determine max available hours
                config = shift_config_map.get(current_day)
                if config and config.working_day:
                    daily_working_hours = config.no_of_shifts * 8
                else:
...
async def update_schedule():
    ...
    try:
        ...
        with db_session:
            ...
            # Step 1: Set inactive orders to NULL
            for order in all_orders:
                if order.project and order.production_order not in active_production_orders:
                    order.project.priority = None
            ...
            # Step 3: Assign priorities to active orders that don't have one
            for order in all_orders:
                if order.project and order.production_order in active_production_orders:
                    if order.project.priority is None:
                        max_priority += 1
                        order.project.priority = max_priority
