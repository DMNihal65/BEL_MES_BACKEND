...
class Project(db.Entity):
    ...
    id = PrimaryKey(int, auto=True)
    name = Required(str)
    priority = Optional(int)  # Changed to Optional to allow null values
...
class ShiftHoursConfiguration(db.Entity):
    _table_ = ("master_order", "shift_hours_configuration")
    id = PrimaryKey(int, auto=True)
    date = Required(datetime)
    working_day = Required(bool)
    no_of_shifts = Required(int)